...
# client-gen), output goes to a gitignored client/ scratch dir with no
# version stamp, for sanity-checking generator output/config by hand. In
# CI (.github/workflows/publish-client.yml), CLIENT_GEN_VERSION and
# CLIENT_GEN_OUT_DIR are set to stamp the package.json version from the
# release tag before `npm publish`.

cd "$(dirname "${BASH_SOURCE[0]}")/.."

OUT_DIR="${CLIENT_GEN_OUT_DIR:-client}"
