...
# floci or CI deploy (not fixed local names) - func-setup.sh writes them to
# .env.floci, which we source below; ci.yml sets them itself for its real
# deployed stack, so only source .env.floci when it exists.
