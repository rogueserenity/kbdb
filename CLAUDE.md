...
**Substantial task scripts live in `scripts/`, referenced from `mise.toml` via `file = "scripts/<name>.sh"`**, not embedded inline as a `run = """..."""` string — mise supports both, but a real standalone `.sh` file gets normal shell tooling (editor syntax highlighting, `shellcheck` runs directly against it without extraction) that an inline TOML string block doesn't. `mise run lint` includes an explicit `shellcheck scripts/*.sh` pass for this reason (`actionlint`'s own shellcheck integration only covers workflow files' inline `run:` blocks, not standalone scripts). Short one-or-two-line tasks (e.g. `test`, `build`) stay inline — this split isn't a hard rule, just proportional to script length.
